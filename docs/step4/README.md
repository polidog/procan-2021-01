...
「確率は整数で重み付け」に関しても「景品に設定されている」という言葉抜けています。
ここは「景品には整数で確率の値が設定される」という定義のほうがしっくりきますね。
...
一旦定数として定義するようにしておいて、あとでなにかあっても対応ができるようにします。
...
※できる限りテストは書いたほうがいいのですが、今回は重要でない箇所のテストを省略します。
