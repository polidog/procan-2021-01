...
大まかに説明すると、パスワードをDBに格納しておき、ユーザーが入力したパスワードとDBに格納されているパスワードがあっているをチェックする。
...
## セッション認証とはなにか？
一言で表すと  
** CookieにセッションIDを保持して、セッションIDを元にユーザーを特定させる認証方法 **   
になるかと思います。
...
HTTPリクエスト時にブラウザはCookieを送信します。そのクッキーのなかに入っているセッションIDを使ってログインしているユーザーかを認証します。
...
これだと見た目がおかしいので修正します。  
以下のコマンドを入力することでbootstrapな見た目のUIになるかと思います。
...
※ npm 関連はdocker経由だと遅いので、ローカルのものを使ったほうが良いかもしれません...
...
簡単に機能を用意できましたが、実際はどのようにログイン認証を行っているのでしょうか？
少しコードをみてみましょう。
...
更にコードを読み進めていく`Illuminate\Auth::attempt()`で、認証処理をしていることがわかります。
...
コマンドで簡単に認証の機能を用意することは出来ますが、ちゃんと理解するためにも一度はコードリーディングを行うことが重要です。
